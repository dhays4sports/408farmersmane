...
export const RC_SMS_CONNECTION_BUILD = 'RC-SMS-1.9.7';
...
export async function handleRingCentralWebhook(request, options = {}) {
  ...
  try {
    ...
    const completedWorkflowReentry = conversation.orchestration?.workflow?.status === 'completed'
      ...
      && ['buyer', 'home_review', 'bundle', 'other', 'auto', 'life', 'business'].includes(routeDecision.intent);
    if (completedWorkflowReentry) {
      const workflowType = ({
        ...
        auto: 'coveragefit_auto_intake',
        life: 'coveragefit_life_intake',
        business: 'coveragefit_business_intake'
    ...
    if (Object.prototype.hasOwnProperty.call(routed, 'entryPoint')) conversation.entryPoint = SMS_LINK_ENTRY_POINTS.includes(text(routed.entryPoint).toLowerCase()) ? text(routed.entryPoint).toLowerCase() : '';
