...
import { enqueue as enqueueProducerNotification } from './pvx-notification-orchestrator.mjs';
...
async function enqueueContactUpgrade(store, webJourney, smsJourney, record, now) {
  const journeyId = text(webJourney?.record?.journeyId || smsJourney?.record?.journeyId || record?.attribution?.webJourneyId || record?.checkpointId, 120);
  if (!journeyId || !store?.get || !store?.setJSON) return null;
  const subjectRef = text(record?.attribution?.techLeadCheckpointId || webJourney?.record?.seed?.identity?.techLeadCheckpointId || record?.checkpointId, 160);
  try {
    return await enqueueProducerNotification(store, {
      journeyId,
      event:'contact_requested',
      subjectRef,
      ownerId:text(webJourney?.record?.seed?.ownership?.producerId, 80) || 'dylan',
...
export async function handlePVXCheckpoint(request, { store, journeyStore, operationsStore, now = new Date(), cryptoApi = globalThis.crypto } = {}) {
  ...
  if (action === 'continue' || action === 'contact_only') {
    ...
    if(contactRequested)await enqueueContactUpgrade(store,webJourney,smsJourney,record,now);
    ...
    if(webJourney)await advancePvxWebJourney(webJourney,{store:journeyStore,now,stage:contactRequested?'contact_requested':'snapshot_completed',currentStep:contactRequested?'contact_requested':'continuing',details});
  ...
  if (action === 'contact') {
    ...
    await enqueueContactUpgrade(store,webJourney,smsJourney,record,now);
    ...
    if(webJourney)await advancePvxWebJourney(webJourney,{store:journeyStore,now,stage:'contact_requested',currentStep:'contact_requested',completedStage:'contact_requested',details});
  ...
  const attribution = { ...(parsed.value?.attribution || {}), ...(smsJourney ? { smsJourneyId: smsJourney.record.journeyId, smsConversationId: smsJourney.record.smsConversationId } : {}), ...(webJourney ? { webJourneyId:webJourney.record.journeyId,techLeadCheckpointId:webJourney.record.seed?.identity?.techLeadCheckpointId||'' } : {}) };
  ...
  if(aliasKey)await store.setJSON(aliasKey,{schemaVersion:'1.0',recordType:'pvx_checkpoint_alias',token,checkpointId:record.checkpointId,createdAt,expiresAt},{metadata:{recordType:'pvx_checkpoint_alias',createdAt,updatedAt:createdAt,expiresAt},onlyIfNew:true});
  if(consent.contact)await enqueueContactUpgrade(store,webJourney,smsJourney,record,now);
  ...
  if(webJourney)await advancePvxWebJourney(webJourney,{store:journeyStore,now,stage:consent.contact?'contact_requested':'snapshot_saved',currentStep:consent.contact?'contact_requested':'saved',completedStage:consent.contact?'contact_requested':'snapshot_saved',details:{reviewTopics:snapshot.whatDylanWouldLookAtFirst,topicResponses:record.topicResponses,preferredContactChannel:contact.preferredMethod,requestedContactTime:contact.bestTime,contactPurpose:contact.purpose,contactQuestion:contact.question,requestedProducerAction:consent.contact?'contact_requested':'none'}});
