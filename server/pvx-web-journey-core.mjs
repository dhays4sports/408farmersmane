...
export const PVX_WEB_JOURNEY_BUILD = 'TECH-PVX-1.10';
...
export const PVX_WEB_STAGES = Object.freeze([
  'entry', 'discovery_started', 'snapshot_viewed', 'snapshot_completed', 'snapshot_saved', 'contact_requested',
  'home_profile_started', 'home_profile_ready', 'policy_review_started',
  'coverage_review_ready', 'policy_review_ready', 'producer_review_ready', 'continue_later'
]);
export const TECH_LEAD_STAGES = Object.freeze(['started','snapshot_completed','contact_requested','home_profile_ready','policy_review_ready']);
...
export function resolvePvxWebDestination(record = {}) {
  ...
  const techRenter = record.seed?.context?.professional?.program === 'technology' && record.seed?.context?.housing?.occupancy === 'renter';
  if (techRenter && ['entry','discovery_started','snapshot_viewed','snapshot_completed','snapshot_saved','contact_requested'].includes(stage)) return '/pvx/start/';
  if (stage === 'discovery_started') return '/pvx/discovery/';
  if (['snapshot_viewed', 'snapshot_completed', 'snapshot_saved', 'contact_requested'].includes(stage)) return '/pvx/snapshot/';
  if (['home_profile_started', 'home_profile_ready'].includes(stage)) return '/pvx/home-profile/';
  if (['policy_review_started', 'coverage_review_ready', 'policy_review_ready'].includes(stage)) return '/pvx/policy/';
...
function publicJourney(record, includeSeed = false) {
  const result = {
    ...
    leadLifecycle: record.leadLifecycle || null,
...
async function persistNewJourney(mapping, sourceOrigin, bootstrapId, options = {}) {
  ...
  const techCheckpointId = text(mapping.identity?.techLeadCheckpointId, 90);
  const startedAt = mapping.identity?.earlyCaptureStatus === 'confirmed'
    ? text(mapping.identity?.consentEvidence?.timestamp, 40) || createdAt : '';
  const record = {
    ...
    completedStages: [],
    projection: {},
    leadLifecycle: techCheckpointId ? {
      schemaVersion:'1.0',
      techLeadCheckpointId:techCheckpointId,
      earlyCaptureStatus:text(mapping.identity?.earlyCaptureStatus, 40),
      stages:startedAt ? [{ stage:'started', occurredAt:startedAt }] : [],
      highestStage:startedAt ? 'started' : '',
      passiveSnapshotNotificationCreated:false
    } : null,
    seed: mapping,
    createdAt,
    updatedAt: createdAt,
...
export async function advancePvxWebJourney(loaded, options = {}) {
  ...
  const lifecycleStage = stage === 'snapshot_viewed' || stage === 'snapshot_completed' ? 'snapshot_completed'
    : stage === 'coverage_review_ready' || stage === 'policy_review_ready' ? 'policy_review_ready'
      : TECH_LEAD_STAGES.includes(stage) ? stage : '';
  const priorLifecycle = loaded.record.leadLifecycle;
  const nextLifecycle = priorLifecycle && lifecycleStage ? {
    ...priorLifecycle,
    stages: priorLifecycle.stages?.some(item => item.stage === lifecycleStage)
      ? priorLifecycle.stages
      : [...(priorLifecycle.stages || []), { stage:lifecycleStage, occurredAt:updatedAt }],
  ...
  const record = {
    ...
    leadLifecycle: nextLifecycle,
    projection: {
      ...(loaded.record.projection || {}),
      ...(['snapshot_viewed','snapshot_completed'].includes(stage) ? { snapshotStatus: 'completed' } : {}),
      ...(stage === 'snapshot_saved' ? { snapshotStatus: 'saved', latestReportRevision: '1' } : {}),
      ...(stage === 'contact_requested' ? { contactStatus:'requested', requestedProducerAction:'contact_requested' } : {}),
      ...
      ...(['coverage_review_ready','policy_review_ready'].includes(stage) ? { policyReviewStatus: 'ready', latestReportRevision: '2P' } : {}),
