...
export const SMS_PRODUCER_HANDOFF_BUILD = 'RC-SMS-1.9.7';
...
function answerLabel(value, labels = {}) { return labels[text(value)] || 'Not captured'; }
...
export function buildSmsProducerSummary(conversation = {}) {
  ...
  const summary = {
    ...
    entryPoint: text(conversation.entryPoint, 'Direct / not captured'),
    ...
    vehicleCount: answerLabel(answers.vehicleCount, { '1': '1', '2': '2', '3': '3', '4_plus': '4+' }),
    driverCount: answerLabel(answers.driverCount, { '1': '1', '2': '2', '3': '3', '4_plus': '4+' }),
    autoReviewReason: answerLabel(answers.autoReviewReason, { renewal_or_price: 'Renewal / price', coverage_question: 'Coverage question', vehicle_or_driver_change: 'Vehicle / driver change', new_policy: 'New policy', other: 'Other' }),
    lifeProtectionGoal: answerLabel(answers.lifeProtectionGoal, { family_income: 'Family income', mortgage: 'Mortgage', final_expenses: 'Final expenses', business: 'Business', not_sure: 'Not sure' }),
    lifeCoveredPerson: answerLabel(answers.lifeCoveredPerson, { self: 'Customer', self_and_family: 'Customer and family / partner', someone_else: 'Someone else', business_person: 'Business partner / key person', not_sure: 'Not sure' }),
    businessIndustry: answerLabel(answers.businessIndustry, { contractor_trades: 'Contractor / trades', restaurant_food: 'Restaurant / food', professional_office: 'Professional / office', retail_service: 'Retail / service', other_or_not_sure: 'Other / not sure' }),
    businessCoverageNeed: answerLabel(answers.businessCoverageNeed, { general_liability: 'General liability', property_equipment: 'Property / equipment', commercial_auto: 'Commercial auto', workers_comp: 'Workers’ compensation', multiple_or_not_sure: 'Multiple / not sure' }),
  ...
  const title = ({
    buyer: 'NEW 408FARMERS BUYER',
    home_review: 'NEW 408FARMERS HOME REVIEW',
    bundle: 'NEW 408FARMERS HOME + AUTO',
    auto: 'NEW 408FARMERS AUTO REVIEW',
    life: 'NEW 408FARMERS LIFE REVIEW',
    business: 'NEW 408FARMERS BUSINESS REVIEW'
  })[conversation.intent] || 'NEW 408FARMERS REQUEST';
  summary.text = [
    title,
    '',
    `Contact: ${summary.buyer}`,
    `Entry point: ${summary.entryPoint}`,
    ...
    ...(conversation.intent === 'auto' ? [`Vehicles: ${summary.vehicleCount}`, `Drivers: ${summary.driverCount}`, `Review reason: ${summary.autoReviewReason}`] : []),
    ...(conversation.intent === 'life' ? [`Protection goal: ${summary.lifeProtectionGoal}`, `Considering coverage for: ${summary.lifeCoveredPerson}`] : []),
    ...(conversation.intent === 'business' ? [`Business type: ${summary.businessIndustry}`, `Coverage need: ${summary.businessCoverageNeed}`] : []),
...
export function determineGuidedResumeState(conversation = {}) {
  ...
  if (intent === 'buyer') {
    ...
    if (!text(a.vehicleCount)) return 'auto_vehicle_count_requested';
    if (!text(a.driverCount)) return 'auto_driver_count_requested';
    if (!text(a.autoReviewReason)) return 'auto_review_reason_requested';
    return 'awaiting_producer';
  } else if (intent === 'life') {
    if (!text(a.lifeProtectionGoal)) return 'life_protection_goal_requested';
    if (!text(a.lifeCoveredPerson)) return 'life_covered_person_requested';
    return 'awaiting_producer';
  } else if (intent === 'business') {
    if (!text(a.businessIndustry)) return 'business_industry_requested';
    if (!text(a.businessCoverageNeed)) return 'business_coverage_requested';
    return 'awaiting_producer';
