    ...
    if (isHandoff) {
      ...
      if (consentCopy) {
        consentCopy.textContent = permissionConfirmed
          ? 'Your contact permission and details were carried forward from the agency request you already completed.'
    ...
    if (appliedFields.length || isHandoff) {
      ...
      note.textContent = isHandoff
        ? (missingFields.length
          ? 'Your agency-request information is connected. Only the missing fields remain.'
          : 'Your agency-request information is connected. You will not need to enter it again.')
